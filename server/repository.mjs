...
export function createRepository(db) {
  return {
    ...
    joinLive: async (id, deviceId) => unwrap(await db.rpc("join_live", { target: id, device_value: deviceId })),
    liveStatus: async (id) => unwrap(await db.rpc("live_status", { target: id })),
    startLive: async (title, providers, deviceId) => unwrap(await db.rpc("start_live", { title_value: title, agora_available: providers.agoraAvailable, livekit_available: providers.livekitAvailable, device_value: deviceId })),
    endLive: async (id, deviceId) => unwrap(await db.rpc("end_live", { target: id, device_value: deviceId })),
