...
export const api = {
  ...
  sendMessage: (conversationId: string, body: string, requestId: string) => request<Message>(`/conversations/${conversationId}/messages`, { method: "POST", body: { body, requestId } }),
  ...
  signUpload: (itemId: string) => request<{ cloudName: string; apiKey: string; timestamp: number; signature: string; publicId: string; folder: string }>("/uploads/sign", { method: "POST", body: { itemId } }),
