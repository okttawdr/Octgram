...
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types) values('chat-once','chat-once',false,26214400,array['image/avif','image/webp','video/mp4','video/webm']) on conflict(id) do update set public=false,file_size_limit=excluded.file_size_limit,allowed_mime_types=excluded.allowed_mime_types;
create policy chat_once_upload on storage.objects for insert to authenticated with check(bucket_id='chat-once' and owner_id=(select auth.uid())::text and (storage.foldername(name))[1]=(select auth.uid())::text and exists(select 1 from public.conversations c where c.id=((storage.foldername(name))[2])::uuid and (select auth.uid()) in(c.user_a,c.user_b)) and lower(storage.extension(name)) in ('avif','webp','mp4','webm'));
-- Media chat kini permanen: kedua peserta percakapan boleh membaca kapan saja (bukan hanya sekali oleh penerima).
create policy chat_once_read_receiver on storage.objects for select to authenticated using(bucket_id='chat-once' and exists(select 1 from public.messages m join public.conversations c on c.id=m.conversation_id where m.media_path=name and (select auth.uid()) in(c.user_a,c.user_b)));
